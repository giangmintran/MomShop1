...
export class BroadcastNewsComponent extends CrudComponentBase implements OnInit {
  ...
  create() {
    let modal = this.dialogService.open(AddNewsComponent, {
      ...
      width: '70%',
      style: {'max-height': '70%', 'border-radius': '0px'},
  ...
  getTypes() {
    ...
    for (let key in MediaConst.productType) {
      keys.push({ key: key, value: MediaConst.getProductType(key) });
    }
    return keys;
  }
  ...
  getStatus() {
    ...
    for (let key in MediaConst.productStatus) {
      keys.push({ key: key, value: MediaConst.getProductStatus(key) });
    }
    return keys;
  }
    ...
    ref.onClose.subscribe((dataCallBack) => {
      if (dataCallBack?.accept) {
       ...
       this.broadcastService.saveNews(row).subscribe(res => {
         this.messageService.add({
           severity: 'success',
           summary: "Xoá sản phẩm thành công",
           life: 3000,
         })
         this.setPage({ page: 0 })
       }, err => {
